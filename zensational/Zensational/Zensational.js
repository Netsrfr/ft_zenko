...
var file = require('fs').createWriteStream('./image');
s3.getObject(params).createReadStream().pipe(file);

execFile('python3', ['/models/tutorials/image/imagenet/classify_image.py', '--num_top_predictions', '1', '--image_file', './image'], (error, stdout, stderr) => {
	var tags = stdout.toString().replace("\n", "").split(", ");
	var i = 1;
	var tagSet = [];
